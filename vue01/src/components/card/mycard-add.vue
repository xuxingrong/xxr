    ...
    <div class="boxLogin">
      <ul>
         <li v-on:click="chooBank()">
               <label>银行</label>
               <!--<input type="text" class="l_bank" placeholder="选择发卡银行" disabled/>-->
               <span class="l_bank">请选择发卡银行</span>
               <div class="l_icons">
                <i class="lIcon_spc"></i>
              </div>
          </li>
          <li>
               <label>分行</label>
               <input type="text" class="l_fh" placeholder="请填写分行信息" v-on:keyup="s_deleted"/>
               <div class="l_icons"><i v-bind:class="{'l_icon':true,'l_delete':true,hide:'true'}" v-on:click="clear" ></i></div>
          </li>
          <li>
               <label>支行</label>
               <input type="text" class="l_zh" placeholder="请填写支行信息" v-on:keyup="s_deleted"/>
               <div class="l_icons"><i v-bind:class="{'l_icon':true,'l_delete':true,hide:'true'}" v-on:click="clear" ></i></div>
          </li>
          <li v-on:click="chooType()">
               <label>类型</label>
               <span class="bankType">选择银行卡类型</span>
               <div class="l_icons">
                 <i class="lIcon_spc"></i>
              </div>
          </li>
          <li>
               <label>姓名</label>
               <input type="text" class="l_name" placeholder="持卡人姓名" v-on:keyup="s_deleted"/>
               <div class="l_icons"><i v-bind:class="{'l_icon':true,'l_delete':true,hide:'true'}" v-on:click="clear" ></i></div>
          </li>
          <li>
               <label>卡号</label>
               <input type="number" class="l_number" id="bNum" placeholder="银行卡号" v-on:keyup="s_deleted"/>
               <div class="l_icons"><i v-bind:class="{'l_icon':true,'l_delete':true,hide:'true'}" v-on:click="clear" ></i></div>
          </li>
       </ul>
...
export default {
  ...
  parent2E:function(){
   ...
   parentE:function(bank_name){
       $('.l_bank').text(bank_name);
       if(bank_name=="" || bank_name==null){
        $('.l_bank').text("请选择发卡银行").css("color","#a5a5a5");
      }else{
        $('.l_bank').text(bank_name).css("color","#333");
      }
    },
  anPd: function(){
    var bNum=$("#bNum").val();
    ...
    if(this.GLOBAL.regular.reg_bank.test(band_card)){
      ...
      var arr = ['timestamp','app_key','app','v','identify','bank_card_owner','bank_card_id','bank_name','bank_name_fenhang','bank_name_branch','set_default'];
    ...
    chooBank: function(){
      this.$router.push('/hhk/mycard-add/bank');
    },
    chooType:function(){
